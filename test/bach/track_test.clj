  ...
  (testing "non-default time signature"
    (testing "3/4"
      (let [tree [:track [:statement [:header [:meta "Time"] [:meter [:number "3"] [:number "4"]]]] [:statement [:list [:pair [:number "1"] [:atom [:keyword "Note"] [:init [:arguments [:string "'C2'"]]]]]]]]
        ...
        (is (= want (get-ms-per-beat tree)))))
    (testing "6/8"
      (let [tree [:track [:statement [:header [:meta "Time"] [:meter [:number "6"] [:number "8"]]]] [:statement [:list [:pair [:number "1"] [:atom [:keyword "Note"] [:init [:arguments [:string "'C2'"]]]]]]]]
            want 1500.0]
        (is (= want (get-ms-per-beat tree)))))))
...
(deftest dereference
  ...
  (testing "variables (nested list/array)"
    (let [tree [:track [:statement [:assign [:identifier :A] [:atom [:keyword "Chord"] [:init [:arguments [:string "'D2min7'"]]]]]]
                       [:statement [:assign [:identifier :B] [:list [:pair [:number "1"] [:identifier :A]]]]]]
          want [:track [:statement [:assign [:identifier :A] [:atom [:keyword "Chord"] [:init [:arguments [:string "'D2min7'"]]]]]]
                       [:statement [:assign [:identifier :B] [:list [:pair [:number "1"] [:atom [:keyword "Chord"] [:init [:arguments [:string "'D2min7'"]]]]]]]]]]
      (is (= want (deref-variables tree)))))
  ; (testing "beats"
  ...
  (testing "measures"
    ...
    (testing "with quarter notes"
      (let [tree [:track [:statement [:assign [:identifier ":ABC"] [:list [:pair [:div [:number "1"] [:number "4"]] [:atom [:keyword "Chord"] [:init [:arguments [:string "'D2min7'"]]]]] [:pair [:div [:number "1"] [:number "2"]] [:atom [:keyword "Chord"] [:init [:arguments [:string "'G2Maj7'"]]]]] [:pair [:div [:number "1"] [:number "4"]] [:atom [:keyword "Chord"] [:init [:arguments [:string "'C2maj7'"]]]]]]]] [:statement [:play [:identifier ":ABC"]]]]
            ...
            ; want [[{:duration 1, :notes {:atom {:keyword "Chord", :init {:arguments ["D2min7"]}}}} {:duration 2, :notes {:atom {:keyword "Chord", :init {:arguments ["G2Maj7"]}}}} nil {:duration 1, :notes {:atom {:keyword "Chord", :init {:arguments ["C2maj7"]}}}}]]]
        (is (= want (normalize-measures tree)))))
